...
//GBFS
Node* Search(Node* initialState, Node* goal) {
    cout<<"Number of Vertices : "<<numVertices<<endl;
    cout<<"Number of Edges : "<<numEdges<<endl;
    cout<<"Start=("<<initialState->v[1]<<", "<<initialState->v[2]<<"), Goal=(";
    cout<<goal->v[1]<<", "<<goal->v[2]<<"), Vertices: "<<initialState->v[0]<<" and "<<goal->v[0]<<endl;
    priority_queue<Node*, vector<Node*>, compare> frontier;
    REP(i, MAX) {
        int deltaX = goal->v[1] - node[i].v[1];
        int deltaY = goal->v[2] - node[i].v[2];
        node[i].heur = deltaX * deltaX + deltaY * deltaY;
    }
    frontier.push(initialState);
    initialState->depth = 0;
    initialState->parent = NULL;
    isVisited[initialState->v[0]] = true;
    iterations = 0;
    verticesVisited = 1;
    ...
    while (!frontier.empty()) {
        ...
        cout<<current->v[1]<<", "<<current->v[2]<<"), depth="<<current->depth<<", dist2goal="<<current->heur<<endl;
        if(current == goal)
            return goal;
        ...
        for(Node* nod : successorList) {
            ...
            } else if(current->depth+1 < nod->depth) {
                nod->depth = current->depth + 1;
                nod->parent = current;
            }
        }
